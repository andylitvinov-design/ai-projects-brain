...
| --- | --- | --- | --- |
| Safe | `/safe`, `режим safe`, `проверь безопасность`, `чтобы пользователь не видел ошибок` | A project needs privacy, API-cost, bot-protection, auth, headers, frontend runtime-error, user-facing error-safety, rollback, observability, or agent-skill safety checks. Route first with `systems/safe-routing.md`; add durable new ideas to `systems/safe-concept.md`. | `systems/safe-mode.md` |
