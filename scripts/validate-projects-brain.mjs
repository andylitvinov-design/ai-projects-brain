...
const SECRET_PATTERNS = [
  { name: 'OpenAI-style secret token', regex: /\bsk-[A-Za-z0-9_-]{16,}/ },
  ...
  { name: 'private_key assignment', regex: /\bprivate_key\s*[:=]\s*['"]?[^'"\s][^'"]{8,}/i },
  { name: 'client_secret assignment', regex: /\bclient_secret\s*[:=]\s*['"]?[^'"\s][^'"]{8,}/i },
  { name: 'API_KEY assignment', regex: /\b[A-Z0-9_]*API_KEY\s*=\s*['"]?[^'"\s][^'"]{8,}/ },
    ...
    lines.forEach((line, index) => {
      for (const pattern of SECRET_PATTERNS) {
        if (pattern.regex.test(line)) {
          fail(`Possible secret value in ${relative}:${index + 1} (${pattern.name})`);
        }
      }
    });
  }
...
for (const project of database.projects) {
  ...
  for (const fileName of REQUIRED_MEMORY_FILES) {
    ...
    if (!fs.existsSync(requiredPath)) {
      fail(`Missing required memory file: projects/${slug}/${fileName}`);
    }
  }
...
if (index.projects.length !== database.projects.length) {
  fail('data/project-index.json project count does not match projects.json');
}
