  ...
  projects: database.projects.map((project) => {
    ...
    return {
      ...
      memory_files: {
        ...
        system_map: memory.system_map || `projects/${slug}/SYSTEM_MAP.md`,
        data_schema: memory.data_schema || `projects/${slug}/DATA_SCHEMA.md`,
        code_access: memory.code_access || `projects/${slug}/CODE_ACCESS.md`,
        data_samples: memory.data_samples || `projects/${slug}/DATA_SAMPLES.md`,
        debug_log: memory.debug_log || `projects/${slug}/DEBUG_LOG.md`,
        risks: memory.risks || `projects/${slug}/RISKS.md`,
        codex_brief: memory.codex_brief || `projects/${slug}/CODEX_BRIEF.md`,
      ...
      verification_flags: {
        ...
        repo_mapping: project.repo_url && project.repo_url !== 'unknown' ? 'listed' : 'needs verification',
        live_mapping: project.live_url && project.live_url !== 'unknown' && project.live_url !== 'not applicable' ? 'listed' : 'needs verification',
        env_status: hasNeedsVerification(project.env_variables) ? 'needs verification' : 'names listed',
        deploy_status: project.hosting && project.hosting !== 'needs verification' ? 'listed' : 'needs verification'
      }
    };
  })
};
...
console.log(`project-index ok: ${index.projects.length} projects -> ${path.relative(ROOT, OUTPUT)}`);
