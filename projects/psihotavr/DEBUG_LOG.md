...
## 2026-07-14 — Project-memory removal regression

- Finding: the project registry still classified Psihotavr as active, but several
  required memory files had been removed as if the project were retired.
- Effect: `validate-projects-brain.mjs` failed and blocked unrelated harness CI.
- Safe fix: restore the previously validated public project-memory files without
  changing product code, providers, data, auth, payments, or env values.
- Verification: run the complete agent harness validator workflow.
