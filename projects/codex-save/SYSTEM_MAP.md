# System Map - codex-save

## 1. High-level Flow

[INPUT] Operator-facing diagnostics/remediation site for
codex-links.

↓

[PROCESSING] needs verification

↓

[STORAGE] needs verification

↓

[OUTPUT] https://codex-save-cjb.pages.dev

## 2. Main Actors

- user
- admin
- provider
- backend
- external APIs
- needs verification

## 3. Data Flow

needs verification

## 4. Runtime Flow

Related subproject under codex-links/codex-save.

## 5. Deploy Flow

https://github.com/andylitvinov-design/codex-links ->
Cloudflare Pages project codex-save ->
https://codex-save-cjb.pages.dev. Exact deploy source needs
verification.

## 6. Critical Paths

- codex-save/README.md
- codex-save/package.json
- codex-save/public/

## 7. Unknowns

- Exact deploy source: needs verification
- Current live behavior: needs verification
- Data flow details not listed in inventory: needs
  verification
