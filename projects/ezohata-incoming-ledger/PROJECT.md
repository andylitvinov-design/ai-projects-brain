...
Web app for EzoHata incoming payments, expenses,
fact data, balances, provider imports, and
channel analytics.
...
Active production source is
andylitvinov-design/finance. Old repo
andylitvinov-design/ezohata-incoming-ledger is
private/deprecated read-only according to GitHub
metadata.
...
- Finance repo PR #5 Harden OCR fallback and
  document finance env.
- Old repo PR #13 Restore full channel
  reconciliation table.

## 10. Next Actions

- Verify open PR status before new production
  work.
- Keep release version and sheet-config.json
  aligned.
- Run npm test, npm run build, and npm run
  release-guard before PR.
